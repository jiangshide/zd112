...
import com.android.widget.refresh.interfaces.RefreshContent;
...
public class RefreshContentWrapper implements RefreshContent, ValueAnimator.AnimatorUpdateListener {
    ...
    protected View findScrollableViewInternal(View content, boolean selfable) {
        ...
        while (!views.isEmpty() && scrollableView == null) {
            ...
            if (view != null) {
                if ((selfable || view != content) && ZdRefreshManager.getInstance().isScrollableView(view)) {
                    scrollableView = view;
    ...
    protected View findScrollableViewByPoint(View content, PointF event, View orgScrollableView) {
        if (content instanceof ViewGroup && event != null) {
            ...
            for (int i = childCount; i > 0; i--) {
                ...
                if (ZdRefreshManager.getInstance().isTransformedTouchPointInView(viewGroup, child, event.x, event.y, point)) {
                    if (child instanceof ViewPager || !ZdRefreshManager.getInstance().isScrollableView(child)) {
    ...
    public void setUpComponent(RefreshKernel kernel, View fixedHeader, View fixedFooter) {
        ...
        if (fixedHeader != null || fixedFooter != null) {
            ...
            if (fixedHeader != null) {
                ...
                lp.height = ZdRefreshManager.getInstance().measureViewHeight(fixedHeader);
            ...
            if (fixedFooter != null) {
                ...
                lp.height = ZdRefreshManager.getInstance().measureViewHeight(fixedFooter);
                parent.addView(new Space(mContentView.getContext()), index, lp);
    ...
    public ValueAnimator.AnimatorUpdateListener scrollContentWhenFinished(final int spinner) {
        if (mScrollableView != null && spinner != 0) {
            if ((spinner < 0 && ZdRefreshManager.getInstance().canScrollDown(mScrollableView)) || (spinner > 0 && ZdRefreshManager.getInstance().canScrollUp(mScrollableView))) {
    ...
    public void onAnimationUpdate(ValueAnimator animation) {
        ...
        try {
            if (mScrollableView instanceof AbsListView) {
                ZdRefreshManager.getInstance().scrollListBy((AbsListView) mScrollableView, value - mLastSpinner);
            } else {
