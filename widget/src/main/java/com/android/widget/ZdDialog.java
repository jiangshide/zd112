...
public class ZdDialog extends Dialog implements AdapterView.OnItemClickListener, ZdWheel.WheelOnItemSelectedListener{
    ...
    private ZdAdapter<String> mCommAdapter;
    ...
    public static ZdDialog createList(Context context, List<String> dataList, List<Integer> dataColorList) {
    ...
    public static ZdDialog createList(Context context, int style, List<String> dataList, List<Integer> dataColorList) {
        cancelDialog();
    ...
    public static ZdDialog createWheel(Context context, List<List<String>> dataList, boolean isBirthday) {
        cancelDialog();
    ...
    public static ZdDialog createWheel(Context context, int style, List<List<String>> dataList, boolean isBirthday) {
        cancelDialog();
    ...
    public ZdDialog(@NonNull Context context, int style, List<String> dataList, List<Integer> dataColorList) {
        super(context, style);
        initList(context, dataList, dataColorList);
    }
    ...
    public ZdDialog(@NonNull Context context, List<List<String>> dataList, boolean isBirthday, boolean isWheel) {
        super(context, R.style.DialogTheme);
        this.mIsBirthday = isBirthday;
        initWheel(context, dataList, isWheel);
    }

    public ZdDialog(@NonNull Context context, int style, List<List<String>> dataList, boolean isBirthday, boolean isWheel) {
        super(context, style);
        this.mIsBirthday = isBirthday;
        initWheel(context, dataList, isWheel);
    }
    ...
    public ZdDialog(@NonNull Context context, int style, View view, DialogViewListener dialogViewListener) {
        super(context, style);
        this.mView = view;
        this.mDialogViewListener = dialogViewListener;
    }
    ...
    public ZdDialog(@NonNull Context context, int style, int layout, DialogViewListener dialogViewListener) {
        super(context, style);
        this.mView = LayoutInflater.from(context).inflate(layout, null);
        this.mDialogViewListener = dialogViewListener;
    }
    ...
    private void defailtWheel() {
        ...
        for (List<String> list : mWheelDatas) {
            ...
            linearLayout.addView(zdWheel, new LinearLayout.LayoutParams(0, ScreenUtil.getScreenHeight(getContext()) / 3, 1));
...
//                    }
    ...
    private void defailtViewList() {
        ...
        mDefaultDialogList.setAdapter(mCommAdapter = new ZdAdapter<String>(getContext(), mDataList, R.layout.default_dialog_list_item) {
            @Override
            protected void convertView(int position, View item, String s) {
                ...
                if (null != mDataColorList && mDataColorList.size() - 1 >= position) {
                    textView.setTextColor(ContextCompat.getColor(getContext(), mDataColorList.get(position)));
                }
                textView.setText(s);
    ...
    private void defaultView() {
        ...
        if (mIsOnlySure) {
            mDialogRight.setCurrCorner(0, 0, getContext().getResources().getDimension(R.dimen.default_corner), getContext().getResources().getDimension(R.dimen.default_corner));
                ...
                if (mDialogOnClickListener != null) {
                    ...
                    if (!isCancel && mIsEdit && null != mDialogEdit) {
                        ...
                        if (TextUtils.isEmpty(editStr)) {
                            ...
                            AnimUtil.shakeView(mDialogEdit);
                            return;
                        }
                    }
    ...
    private boolean isCancel(ZdButton zdButton) {
        ...
        if (!TextUtils.isEmpty(btnStr) && (btnStr.contains(getContext().getString(R.string.cancel)) || btnStr.contains(getContext().getString(R.string.off)))) {
            return true;
        }
        return false;
    }
    ...
    public ZdDialog setRightTxtColor(int color) {
        if (null != mDialogRight) mDialogRight.setTextColor(ContextCompat.getColor(getContext(), color));
        return this;
    }
    ...
    public static void popuWindow(Context context, View view, int layout, OnPopuListener listener) {
        ...
        if (ScreenUtil.getScreenHeight(context) - SystemUtil.getStatusBarHeight((Activity) context) - location[1] < ScreenUtil.dip2px(context, 80)) {
            mPopupWindow.showAsDropDown(view, ScreenUtil.getScreenWidth(context) / 4 - (int) ScreenUtil.dip2px(context, 40), -view.getHeight() - (int) ScreenUtil.dip2px(context, 80));
            isBottom = true;
        } else {
            mPopupWindow.showAsDropDown(view, ScreenUtil.getScreenWidth(context) / 4 - (int) ScreenUtil.dip2px(context, 40), 0);
            isBottom = false;
        }
        if (listener != null) {
