...
class IgnoreHostProxySelector extends ProxySelector {
    ...
    IgnoreHostProxySelector(ProxySelector proxySelector, String hostToIgnore, int portToIgnore) {
    ...
    public List<Proxy> select(URI uri) {
        boolean ignored = hostToIgnore.endsWith(uri.getHost()) && portToIgnore == uri.getPort();
        return ignored ? NO_PROXY_LIST : proxySelector.select(uri);
    }
